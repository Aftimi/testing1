...
$query_requete= mysql_query($requete) OR die('Requête invalide : ' . mysql_error());
   ...
   echo "<H2 style='color=black'><table><H2 style='color=black'><tr><td>$data[7]</td><td>$data[2]</td><td>$data[3]</td></tr><tr><td></td><td><center><center><form method='POST' action='repcommentad.php?idp=$data[0]&id=$data[1]'><input type='text' id='newcom' name='newcom' placeholder='votre repondre...'></center></center></td><td><input type='submit' class='btn btn-info' value='repondre'></form></td></tr></table></h2>";
$requete = "SELECT commentaire.id_pub,commentaire.id_com,repcommentaire.id_new,repcommentaire.id_pub,repcommentaire.id_com,repcommentaire.id_pers,repcommentaire.contenu,repcommentaire.date_com,personne.id_pers,personne.nom FROM personne,commentaire,repcommentaire where commentaire.id_pub=repcommentaire.id_pub  and repcommentaire.id_pers=personne.id_pers ";
$query_requete= mysql_query($requete) OR die('Requête invalide : ' . mysql_error());
    echo"<div class='container2'>";
...
echo "<table><H2 style='color=black'><tr><td>$dataa[9]</td><td>$dataa[6]</td><td>$dataa[7]</td></tr><tr><td></td><td><center><a href='suprepadmin.php?idp=$data[0]&id=$dataa[1]&idr=$dataa[2]'>Supprimer repondre</a><center></td></tr></H2></table></center></br>";
echo"</center>";}
mysql_close();
?>
